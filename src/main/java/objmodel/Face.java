...
public class Face
{
    ...
    @SideOnly(Side.CLIENT)
    public void addFaceForRenderRate(Tessellator2 tessellator, float textureOffset, Face targetIn, float rateIn)
    {
        if (faceNormal == null)
        {
            faceNormal = this.calculateFaceNormal();
        ...
        for (int i = 0; i < vertices.length; ++i)
        {
            float newX = vertices[i].x + (targetIn.vertices[i].x - vertices[i].x) * rateIn;
            float newY = vertices[i].y + (targetIn.vertices[i].y - vertices[i].y) * rateIn;
            float newZ = vertices[i].z + (targetIn.vertices[i].z - vertices[i].z) * rateIn;
...
//                tessellator.addVertexWithUV(vertices[i].x, vertices[i].y, vertices[i].z, textureCoordinates[i].u + offsetU, textureCoordinates[i].v + offsetV);
                tessellator.addVertexWithUV(newX, newY, newZ, textureCoordinates[i].u + offsetU, textureCoordinates[i].v + offsetV);
            }
            else
            {
//                tessellator.addVertex(vertices[i].x, vertices[i].y, vertices[i].z);
                tessellator.addVertex(newX, newY, newZ);
            }
        }
    }
